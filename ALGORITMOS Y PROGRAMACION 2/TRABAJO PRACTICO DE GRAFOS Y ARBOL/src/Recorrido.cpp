...
void Recorrido::reasignar_peso(std::vector<size_t> coordenada_pyramid)
{
    std::vector<std::vector<size_t>> layout = numero_layout == 1 ? VERTICES_LAYOUT1 : VERTICES_LAYOUT2;
...
void Recorrido::actualizar_valor_arista_pyramidhead(size_t vertice_pyramid)
{
    if (numero_layout == 1)
    {
        reasignar_peso(VERTICES_LAYOUT1[vertice_pyramid]);
    }
    else
    {
        reasignar_peso(VERTICES_LAYOUT2[vertice_pyramid]);
